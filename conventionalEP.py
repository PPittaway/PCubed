from PyQt5 import QtWidgets
import numpy as np
import pandas as pd
import datetime
import time
import threading
import DLS_handler
import random

class conventionalEP(QtWidgets.QWidget):
    def __init__(self, parent, main):
        super(conventionalEP, self).__init__(parent)
       
        self.main = main

    def start(self, method="standard"):
        if method=="standard":
            expThread = threading.Thread(target=self.runScript)
            expThread.start()
        if method=="kinetics":
            expThread = threading.Thread(target=self.runKinetics)
            expThread.start()

    def runScript(self, optimisationFlag=1):
        expID = self.main.methodHandler.experimentIDLineEdit.text()
        expIndex = self.main.methodHandler.expIndex
        parameters = self.main.methodHandler.variablesDict
        if optimisationFlag:
            sampleStrategy = int(self.main.methodHandler.sampleStrategyText.text()) # If an optimisation, sample according to the defined strategy (every x reactions)
        else: 
            sampleStrategy = 1 # Else sample every reaction
        reactionsCounter = 0
        sampleLoopCounter = 0
        experimentLog = {}
        if optimisationFlag:
            optimisationSummary = self.main.methodHandler.optimisationSummary
        
        ### Extract parameters
        volumeCSTR = parameters.get("volumeCSTR")
        numberCSTRs = parameters.get("numberCSTRs")
        volumeReactor2 = parameters.get("volumeReactor2")
        deadVolume = parameters.get("deadVolume")[0]
        cleanRate = parameters.get("cleanRate")[0]
        cleanTime = parameters.get("cleanTime")[0]
        cleanVolume = parameters.get("cleanVolume")[0]
        nEmulsionFeeds = parameters.get("numEmulsionFeeds")
        nEmulsionFeedsActive = max(nEmulsionFeeds)

        ### Name all equipment
        seedPump = self.main.controller.pump1
        monomerAPump = self.main.controller.pump2
        monomerBPump = self.main.controller.pump3
        surfactantPump = self.main.controller.pump12
        initiatorPump = self.main.controller.pump11
        waterPump = self.main.controller.pump9
        DLSpump = self.main.controller.pump10
        solventPump = self.main.controller.pump6

        solventValve = self.main.controller.valve1
        emulsionValve = self.main.controller.valve2
        outletValve = self.main.controller.valve5
        self.DLS = DLS_handler.StoppedFlowDLS(self, main=self.main)

        ### Get flowrates for pre-programmed conditions (OFAAT, DoE, LHC or first optimisation experiment)
        v_seed = self.main.methodHandler.flowRates.get("v_seed")
        v_monomerA = self.main.methodHandler.flowRates.get("v_monomerA")
        v_monomerB = self.main.methodHandler.flowRates.get("v_monomerB")
        v_surfactant = self.main.methodHandler.flowRates.get("v_surfactant")
        v_initiator = self.main.methodHandler.flowRates.get("v_initiator")
        v_water = self.main.methodHandler.flowRates.get("v_water")

        ######################
        ###   PREPARATION  ###
        ######################

        ''' Start main script '''
        print(f'Starting experiment at {str(datetime.datetime.now())}')
        logEvent = f"Starting experiment {str(expID)}"
        logEventTime = str(datetime.datetime.now())
        experimentLog["Event"] = list([logEvent])
        experimentLog["Time"] = list([logEventTime])

        outletValve.valveHome() # set outlet to waste
        emulsionValve.valveHome() # set emulsion valve to waste
        solventValve.valveHome() # set solvent to surfactant position

        ''' Run solvent pump '''
        solventPump.setFlowrateText.setText(str(cleanRate))
        solventPump.setFlowrate()
        solventPump.start()
        
        print('Priming emulsion feeds...')
        ''' Prime emulsion feeds with surfactant solution (10 s for each) '''
        for i in range(2, int(nEmulsionFeedsActive) + 2):
            emulsionValve.valveSwitch(position=i)
            time.sleep(10)
        for i in reversed(range(2, int(nEmulsionFeedsActive) + 2)):
            emulsionValve.valveSwitch(position=i)
            time.sleep(10)

        solventPump.stop()

        ''' Update waste volume available '''
        volumePriming = 2*20*nEmulsionFeedsActive*(cleanRate/60)
        self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - volumePriming
        self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))

        emulsionValve.distributionModeCheckbox.setChecked(True) # Activate distribution mode on emulsion feed valve

        ''' Write script/loop here '''
        #######################
        ### PROGRAMMED LOOP ###
        #######################
        for i in expIndex.astype(int):
            reactionsCounter+=1
            ''' define sample ID '''
            sampleID = f'{expID}-{str(reactionsCounter)}'

            ''' First calculate the total flow rates, time required to steady-state, and waste volume space required '''
            total_flowrate_i = v_seed[i] + v_monomerA[i] + v_surfactant[i] + v_initiator[i] + v_water[i]
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Add monomer B flowrate if two monomer feeds being used
                total_flowrate_i += v_monomerB[i]

            time_SS = 3*((volumeCSTR[i]*numberCSTRs[i]) + volumeReactor2[i] + deadVolume)/total_flowrate_i # Time to reach steady state (min)

            wasteVolumeReaction = time_SS*total_flowrate_i # Waste volume generated from getting to steady state (mL)
            wasteVolumeCleaningStep = cleanVolume # Waste volume generated from a single cleaning step
            wasteVolumeRequired = wasteVolumeReaction + 3*wasteVolumeCleaningStep # Total volume of waste generated in the i'th experiment (mL)

            preparationTime = 60*3*volumeCSTR[i]/(10*(v_monomerA[i] + v_monomerB[i] + v_surfactant[i] + v_initiator[i] + v_water[i]))
            sampleTime = 60*parameters.get("volumeSample")[i]/total_flowrate_i

            if wasteVolumeRequired > self.main.methodHandler.wasteVolumeAvailable:
                self.main.methodHandler.pauseExperiment = True
                self.main.methodHandler.dialogueLabel.setText("WARNING - waste bottle too full to complete\nthe next experiment, please empty it to continue")
                self.main.methodHandler.dialogueLabel.setHidden(False)
                self.main.methodHandler.ackButton.setHidden(False)
                while self.main.methodHandler.pauseExperiment:
                    time.sleep(1)
                self.main.methodHandler.wasteVolumeAvailable = 970 # Assumes waste bottle has been emptied and resets to full capacity
            
            ''' Update waste bottle volume available '''
            self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))

            seedVolumeRequired = (10*v_seed[i]*preparationTime/60) + (v_seed[i]*time_SS)
            if optimisationFlag and reactionsCounter%sampleStrategy == 0: # check if a sample will be taken to calculate correct volume required
                seedVolumeRequired += (v_seed[i]*sampleTime/60)
        
            if seedVolumeRequired > self.main.methodHandler.seedVolumeAvailable:
                self.main.methodHandler.pauseExperiment = True
                self.main.methodHandler.dialogueLabel.setText("WARNING - seed syringe too empty for the next reaction, please refill and press continue")
                self.main.methodHandler.dialogueLabel.setHidden(False)
                self.main.methodHandler.ackButton.setHidden(False)
                while self.main.methodHandler.pauseExperiment:
                    time.sleep(1)
                self.main.methodHandler.seedVolumeAvailable = 95 # Assumes waste bottle has been emptied and resets to full capacity

            ''' Prepare emulsion at fast flow rate before starting reaction and start filling reactor with seed '''
            print(f'Preparing emulsion for {sampleID}')
            emulsionValve.valveHome()
            seedPump.setFlowrateText.setText(str(np.round(10*v_seed[i], 5)))
            monomerAPump.setFlowrateText.setText(str(np.round(10*v_monomerA[i], 4)))
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.setFlowrateText.setText(str(np.round(10*v_monomerB[i], 4)))
            surfactantPump.setFlowrateText.setText(str(np.round(10*v_surfactant[i], 4)))
            initiatorPump.setFlowrateText.setText(str(np.round(10*v_initiator[i], 4)))
            waterPump.setFlowrateText.setText(str(np.round(10*v_water[i], 4)))

            seedPump.setFlowrate()
            monomerAPump.setFlowrate()
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.setFlowrate()
            initiatorPump.setFlowrate()
            surfactantPump.setFlowrate()
            waterPump.setFlowrate()
            
            seedPump.start()
            monomerAPump.start()
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.start()
            initiatorPump.start()
            surfactantPump.start()
            waterPump.start()

            emulsionValve.valveHome()

            ''' Calculate and start timer to prepare emulsion (3 CSTR volumes at preparation flow rate) '''
            startPreparationTime = datetime.datetime.now()
            while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startPreparationTime).seconds < preparationTime:
#############
                self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (10*v_seed[i]/60)
                self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                time.sleep(1)

            logEvent = f"Starting reaction {sampleID}"
            logEventTime = datetime.datetime.now()
            experimentLog["Event"].append(logEvent)
            experimentLog["Time"].append(str(logEventTime))

            if self.main.methodHandler.manualStop:
                break

            ''' Set flow rates '''
            seedPump.setFlowrateText.setText(str(v_seed[i]))
            monomerAPump.setFlowrateText.setText(str(v_monomerA[i]))
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.setFlowrateText.setText(str(v_monomerB[i]))
            surfactantPump.setFlowrateText.setText(str(v_surfactant[i]))
            initiatorPump.setFlowrateText.setText(str(v_initiator[i]))
            waterPump.setFlowrateText.setText(str(v_water[i]))
            seedPump.setFlowrate()
            monomerAPump.setFlowrate()
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.setFlowrate()
            initiatorPump.setFlowrate()
            surfactantPump.setFlowrate()
            waterPump.setFlowrate()

            emulsionValve.nFeedsLineEdit.setText(str(nEmulsionFeeds[i])) # Set number of feeds for emulsion distribution
            emulsionValve.startDistribution()

            print(f"Starting synthesis, steady state expected at {str(datetime.datetime.now() + datetime.timedelta(seconds=60*time_SS))}")

            '''Wait for steady state'''
            start_exp_time = datetime.datetime.now()
            while not self.main.methodHandler.stopThread and (datetime.datetime.now() - start_exp_time).seconds < 60*time_SS:
##############
                self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeReaction/(60*time_SS))
                self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (v_seed[i]/60)
                self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                time.sleep(1)
            if self.main.methodHandler.manualStop:
                break
            
            if optimisationFlag and reactionsCounter%sampleStrategy == 0: # Sample every x reactions if running an optimisation where x = sampleStrategy
                print(f"Collecting product sample for {sampleID}")
                print(f'Sampler switched to port {((reactionsCounter/sampleStrategy) - (sampleLoopCounter*7) + 1)}')


#########                
                outletValve.valveSwitch(position=int((reactionsCounter/sampleStrategy) - (sampleLoopCounter*7) + 1)) # Start collecting sample by switching to next sample position
#########
                '''Calculate and start timer to collect defined amount of sample'''
                sampleTime = 60*parameters.get("volumeSample")[i]/total_flowrate_i
                startSampleTime = datetime.datetime.now()
                logEvent = f"Sample collection for {sampleID} started"
                logEventTime = str(datetime.datetime.now())
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))
                print(f'At steady state, start sampling until {str(startSampleTime + datetime.timedelta(seconds=sampleTime))}')
                while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startSampleTime).seconds < sampleTime:
############
                    self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (v_seed[i]/60)
                    self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                    time.sleep(1)
                logEvent = f"Sample collection for {sampleID} completed"
                logEventTime = datetime.datetime.now()
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))
            elif not optimisationFlag: # If not an optimisation, just sample every experiment
                print(f"Collecting product sample for {sampleID}")
                print(f'Sampler switched to port {((reactionsCounter/sampleStrategy) - (sampleLoopCounter*7) + 1)}')
#########                
                outletValve.valveSwitch(position=((reactionsCounter/sampleStrategy) - (sampleLoopCounter*7) + 1)) # Start collecting sample by switching to next sample position
#########
                '''Calculate and start timer to collect defined amount of sample'''
                sampleTime = 60*parameters.get("volumeSample")[i]/total_flowrate_i
                startSampleTime = datetime.datetime.now()
                logEvent = f"Sample collection for {sampleID} started"
                logEventTime = datetime.datetime.now()
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))
                print(f'At steady state, start sampling until {str(startSampleTime + datetime.timedelta(seconds=sampleTime))}')
                while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startSampleTime).seconds < sampleTime:
############
                    self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (v_seed[i]/60)
                    self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                    time.sleep(1)
                logEvent = f"Sample collection for {sampleID} completed"
                logEventTime = datetime.datetime.now()
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))
            if self.main.methodHandler.manualStop:
                break
            
            '''Start DLS analysis - extract sample, dilute, and pause flow'''
            print("Sampling for DLS")
            self.DLS.sampleAndScan(concentration=parameters.get("w_f")[i], flowRate=total_flowrate_i, sampleID=sampleID)
            logEvent = f"DLS sample {sampleID} extracted"
            logEventTime = datetime.datetime.now()
            experimentLog["Event"].append(logEvent)
            experimentLog["Time"].append(str(logEventTime))
            ''' Sample collected, start cleaning cycle '''
            outletValve.valveHome()
            seedPump.stop()
            monomerAPump.stop()
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.stop()
            initiatorPump.stop()
            surfactantPump.stop()
            waterPump.stop()

            ''' Clean with surfactant solution including emulsion feeds '''
            solventPump.start()

            print('Starting cleaning stage 1 ...')
            startClean1Time = datetime.datetime.now()
            logEvent = f"Cleaning with surfactant solution after reaction {sampleID}"
            logEventTime = startClean1Time
            experimentLog["Event"].append(logEvent)
            experimentLog["Time"].append(str(logEventTime))
            while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startClean1Time).seconds < 60*cleanTime:
#############
                self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeCleaningStep/(60*cleanTime))
                self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                time.sleep(1)

            if self.main.methodHandler.manualStop:
                break

            if self.DLS.DLSDataCollected == 1: # Check if DLS data has been saved and add size value to optimisationSummary
                newSize = self.DLS.zAve
                optimisationSummary["Particle_size"].append(newSize)
                logEvent = f"DLS data saved for {sampleID}"
                logEventTime = datetime.datetime.now()
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))
            else: 
                print("Warning: no DLS data collected for this experiment")
                newSize = random.randint(0, 10)
                optimisationSummary["Particle_size"].append(newSize)
                logEvent = f"DLS data saved for {sampleID}"
                logEventTime = datetime.datetime.now()
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))

            # Update the values of the optimisation objectives which are equal to the minimised inputs
            
            for key in self.main.methodHandler.inputsDict.keys():
                optimisationSummary[f'{key}_min'].append(optimisationSummary[key][i])

            # newSurfactantConcMin = optimisationSummary["Surfactant_concentration"][i]
            # newSeedFracMin = optimisationSummary["Seed_fraction"][i]
            # optimisationSummary["Surfactant_concentration_min"].append(newSurfactantConcMin)
            # optimisationSummary["Seed_fraction_min"].append(newSeedFracMin)

            ''' Clean with solvent '''
            emulsionValve.stopDistribution()
            emulsionValve.valveSwitch(position=2) # Send solvent into CSTR 1
            solventValve.valveSwitch(position=2) # Switch solvent inlet

            print('Starting cleaning stage 2 ...')
            startClean2Time = datetime.datetime.now()
            logEvent = "Cleaning with solvent after reaction " + sampleID
            logEventTime = startClean2Time
            experimentLog["Event"].append(logEvent)
            experimentLog["Time"].append(str(logEventTime))
            while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startClean2Time).seconds < 60*cleanTime:
############
                self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeCleaningStep/(60*cleanTime))
                self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                time.sleep(1)

            if self.main.methodHandler.manualStop:
                break

            ''' Rinse with surfactant solution '''
            solventValve.valveHome() # Switch to surfactant solution

            print('Starting cleaning stage 3 ...')
            startClean3Time = datetime.datetime.now()
            logEvent = f"Refilling with surfactant solution after reaction {sampleID}"
            logEventTime = startClean3Time
            experimentLog["Event"].append(logEvent)
            experimentLog["Time"].append(str(logEventTime))
            while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startClean3Time).seconds < 60*cleanTime:
#############
                self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeCleaningStep/(60*cleanTime))
                self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                time.sleep(1)

            solventPump.stop()
            
            if self.main.methodHandler.manualStop:
                break

            if not self.main.methodHandler.stopThread and (reactionsCounter) % (7*sampleStrategy) == 0:
                self.main.methodHandler.pauseExperiment = True
                self.main.methodHandler.dialogueLabel.setText("Experiment paused, clean sample bottles and press OK to continue")
                self.main.methodHandler.dialogueLabel.setHidden(False)
                self.main.methodHandler.ackButton.setHidden(False)
                sampleLoopCounter += 1 # iterate sampleLoopCounter so the next experiment takes the sample in the correct sampling port
                while self.main.methodHandler.pauseExperiment:
                    time.sleep(1)

            print(f"New optimisationSummary = {optimisationSummary}")
            pd.DataFrame.from_dict(optimisationSummary).to_excel(f'{self.main.methodHandler.savePath}-optimisationSummary.xlsx') # Save optimisation summary to save folder
            pd.DataFrame.from_dict(experimentLog).to_excel(f'{self.main.methodHandler.savePath}-experimentLog.xlsx') # Save experiment log to save folder

            self.main.methodHandler.optIterations +=1 # Iterate the optimisation iterations to keep track of the total number of experiments which make up the optimisation

        print("Finished pre-programmed experiments")
        print(f"Final optimisationSummary = {optimisationSummary}")
        
        ######################
        ### OPTIMISER LOOP ###
        ######################
        if optimisationFlag:
            print("Beginning optimisation")

            while not self.main.methodHandler.stopThread and self.main.methodHandler.optIterations < self.main.methodHandler.maxIterations:
                reactionsCounter+=1
                ''' define sample ID '''
                sampleID = f'{expID}-{str(reactionsCounter)}'

                newVars = self.main.methodHandler.getNewConditions(current_dataSet=optimisationSummary) # Requests a new set of experiments from the methodHandler, with a call to the algorithm and subequently a new set of calculated flow rates
                print(f"optimisation summary after newVars = {optimisationSummary}")
                v_seed = newVars["v_seed"][-1]
                v_monomerA = newVars["v_monomerA"][-1]
                v_monomerB = newVars["v_monomerB"][-1]
                v_surfactant = newVars["v_surfactant"][-1]
                v_initiator = newVars["v_initiator"][-1]
                v_water = newVars["v_water"][-1]
                nEmulsionFeeds = newVars["numEmulsionFeeds"][-1]

                ''' First calculate the total flow rates, time required to steady-state, and waste volume space required '''
                total_flowrate_i = v_seed + v_monomerA + v_surfactant + v_initiator + v_water
                if self.main.methodHandler.monomerRatioCheckBox.isChecked():
                    total_flowrate_i += v_monomerB
                time_SS = 3*((volumeCSTR[0]*numberCSTRs[0]) + volumeReactor2[0] + deadVolume)/total_flowrate_i # Time to reach steady state (min)

                wasteVolumeReaction = time_SS*total_flowrate_i # Waste volume generated from getting to steady state (mL)
                wasteVolumeCleaningStep = cleanVolume # Waste volume generated from a single cleaning step (3 reactor volumes)
                wasteVolumeRequired = wasteVolumeReaction + 3*wasteVolumeCleaningStep # Total volume of waste generated in the i'th experiment (mL)

                if wasteVolumeRequired > self.main.methodHandler.wasteVolumeAvailable:
                    self.main.methodHandler.pauseExperiment = True
                    self.main.methodHandler.dialogueLabel.setText("WARNING - waste bottle too full to complete\nthe next experiment, please empty it to continue")
                    self.main.methodHandler.dialogueLabel.setHidden(False)
                    self.main.methodHandler.ackButton.setHidden(False)
                    while self.main.methodHandler.pauseExperiment:
                        time.sleep(1)
                    self.main.methodHandler.wasteVolumeAvailable = 970 # Assumes waste bottle has been emptied and resets to full capacity
                
                ''' Update waste bottle volume available '''
                self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))

                preparationTime = 60*3*volumeCSTR[0]/(10*(v_monomerA + v_monomerB + v_surfactant + v_initiator + v_water))
                seedVolumeRequired = (10*v_seed*preparationTime/60) + (v_seed*time_SS)
                if optimisationFlag and reactionsCounter%sampleStrategy == 0: # check if a sample will be taken to calculate correct volume required
                    seedVolumeRequired += (v_seed*sampleTime/60)
            
                if seedVolumeRequired > self.main.methodHandler.seedVolumeAvailable:
                    self.main.methodHandler.pauseExperiment = True
                    self.main.methodHandler.dialogueLabel.setText("WARNING - seed syringe too empty for the next reaction, please refill and press continue")
                    self.main.methodHandler.dialogueLabel.setHidden(False)
                    self.main.methodHandler.ackButton.setHidden(False)
                    while self.main.methodHandler.pauseExperiment:
                        time.sleep(1)
                    self.main.methodHandler.seedVolumeAvailable = 95 # Assumes waste bottle has been emptied and resets to full capacity

                ''' Prepare emulsion at fast flow rate before starting reaction and start filling reactor with seed '''
                print(f'Preparing emulsion for {sampleID}')
                emulsionValve.valveHome()
                seedPump.setFlowrateText.setText(str(np.round(10*v_seed, 5)))
                monomerAPump.setFlowrateText.setText(str(np.round(10*v_monomerA, 4)))
                if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                    monomerBPump.setFlowrateText.setText(str(np.round(10*v_monomerB, 4)))
                surfactantPump.setFlowrateText.setText(str(np.round(10*v_surfactant, 4)))
                initiatorPump.setFlowrateText.setText(str(np.round(10*v_initiator, 4)))
                waterPump.setFlowrateText.setText(str(np.round(10*v_water, 4)))

                seedPump.setFlowrate()
                monomerAPump.setFlowrate()
                if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                    monomerBPump.setFlowrate()
                initiatorPump.setFlowrate()
                surfactantPump.setFlowrate()
                waterPump.setFlowrate()
                
                seedPump.start()
                monomerAPump.start()
                if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                    monomerBPump.start()
                initiatorPump.start()
                surfactantPump.start()
                waterPump.start()

                emulsionValve.valveHome()

                ''' Calculate and start timer to prepare emulsion (3 CSTR volumes at preparation flow rate) '''
                startPreparationTime = datetime.datetime.now()
                while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startPreparationTime).seconds < preparationTime:
#############
                    self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (10*v_seed/60)
                    self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                    time.sleep(1)

                logEvent = "Starting reaction " + sampleID
                logEventTime = datetime.datetime.now()
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))

                if self.main.methodHandler.manualStop:
                    break

                ''' Set flow rates '''
                seedPump.setFlowrateText.setText(str(v_seed))
                monomerAPump.setFlowrateText.setText(str(v_monomerA))
                if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                    monomerBPump.setFlowrateText.setText(str(v_monomerB))
                surfactantPump.setFlowrateText.setText(str(v_surfactant))
                initiatorPump.setFlowrateText.setText(str(v_initiator))
                waterPump.setFlowrateText.setText(str(v_water))
                seedPump.setFlowrate()
                monomerAPump.setFlowrate()
                if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                    monomerBPump.setFlowrate()
                initiatorPump.setFlowrate()
                surfactantPump.setFlowrate()
                waterPump.setFlowrate()

                emulsionValve.nFeedsLineEdit.setText(str(nEmulsionFeeds)) # Set number of feeds for emulsion distribution
                emulsionValve.startDistribution()
                
                print(f"Starting synthesis, steady state expected at {str(datetime.datetime.now() + datetime.timedelta(seconds=60*time_SS))}")

                '''Wait for steady state'''
                start_exp_time = datetime.datetime.now()
                while not self.main.methodHandler.stopThread and (datetime.datetime.now() - start_exp_time).seconds < 60*time_SS:
############
                    self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeReaction/(60*time_SS))
                    self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                    self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (v_seed/60)
                    self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                    time.sleep(1)
                if self.main.methodHandler.manualStop:
                    break
                
                if optimisationFlag and reactionsCounter%sampleStrategy == 0: # Sample every x reactions if running an optimisation where x = sampleStrategy
                    print(f"Collecting product sample for {sampleID}")
                    print(f'Sampler switched to port {((reactionsCounter/sampleStrategy) - (sampleLoopCounter*7) + 1)}')
##########                    
                    outletValve.valveSwitch(position=((reactionsCounter/sampleStrategy) - (sampleLoopCounter*7) + 1)) # Start collecting sample by switching to next sample position
##########
                    '''Calculate and start timer to collect defined amount of sample'''
                    sampleTime = 60*parameters.get("volumeSample")[i]/total_flowrate_i
                    startSampleTime = datetime.datetime.now()
                    logEvent = f"Sample collection for {sampleID} started"
                    logEventTime = datetime.datetime.now()
                    experimentLog["Event"].append(logEvent)
                    experimentLog["Time"].append(str(logEventTime))
                    print(f'At steady state, start sampling until {str(startSampleTime + datetime.timedelta(seconds=sampleTime))}')
############
                    while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startSampleTime).seconds < sampleTime:
                        self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (v_seed/60)
############
                        self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                        time.sleep(1)
                    logEvent = f"Sample collection for {sampleID} completed"
                    logEventTime = datetime.datetime.now()
                    experimentLog["Event"].append(logEvent)
                    experimentLog["Time"].append(str(logEventTime))
                elif not optimisationFlag: # If not an optimisation, just sample every experiment
                    print(f"Collecting product sample for {sampleID}")
                    print(f'Sampler switched to port {((reactionsCounter/sampleStrategy) - (sampleLoopCounter*7) + 1)}')
###########
                    outletValve.valveSwitch(position=((reactionsCounter/sampleStrategy) - (sampleLoopCounter*7) + 1)) # Start collecting sample by switching to next sample position
###########
                    '''Calculate and start timer to collect defined amount of sample'''
                    sampleTime = 60*parameters.get("volumeSample")[i]/total_flowrate_i
                    startSampleTime = datetime.datetime.now()
                    logEvent = f"Sample collection for {sampleID} started"
                    logEventTime = datetime.datetime.now()
                    experimentLog["Event"].append(logEvent)
                    experimentLog["Time"].append(str(logEventTime))
                    print(f'At steady state, start sampling until {str(startSampleTime + datetime.timedelta(seconds=sampleTime))}')
############
                    while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startSampleTime).seconds < sampleTime:
                        self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (v_seed/60)
############
                        self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                        time.sleep(1)
                    logEvent = f"Sample collection for {sampleID} completed"
                    logEventTime = datetime.datetime.now()
                    experimentLog["Event"].append(logEvent)
                    experimentLog["Time"].append(str(logEventTime))
                if self.main.methodHandler.manualStop:
                    break
                
                '''Start DLS analysis - extract 
                sample, dilute, and pause flow'''
                print("Sampling for DLS")
                self.DLS.sampleAndScan(concentration=parameters.get("w_f")[i], flowRate=total_flowrate_i, sampleID=sampleID)
                logEvent = f"DLS sample {sampleID} extracted"
                logEventTime = datetime.datetime.now()
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))

                ''' Sample collected, start cleaning cycle '''
                outletValve.valveHome()
                seedPump.stop()
                monomerAPump.stop()
                if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                    monomerBPump.stop()
                initiatorPump.stop()
                surfactantPump.stop()
                waterPump.stop()

                ''' Clean with surfactant solution including emulsion feeds '''
                solventPump.start()

                print('Starting cleaning stage 1 ...')
                startClean1Time = datetime.datetime.now()
                logEvent = "Cleaning with surfactant solution after reaction " + sampleID
                logEventTime = startClean1Time
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))
                while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startClean1Time).seconds < 60*cleanTime:
############                   
                    self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeCleaningStep/(60*cleanTime))
                    self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                    time.sleep(1)

                if self.main.methodHandler.manualStop:
                    break

                if self.DLS.DLSDataCollected == 1: # Check if DLS data has been saved and add size value to optimisationSummary
                    newSize = self.DLS.zAve
                    optimisationSummary["Particle_size"].append(newSize)
                    logEvent = f"DLS data saved for {sampleID}"
                    logEventTime = datetime.datetime.now()
                    experimentLog["Event"].append(logEvent)
                    experimentLog["Time"].append(str(logEventTime))
                else: 
                    print("Warning: no DLS data collected for this experiment")
                    newSize = 0
                    optimisationSummary["Particle_size"].append(newSize)
                    logEvent = f"DLS data saved for {sampleID}"
                    logEventTime = datetime.datetime.now()
                    experimentLog["Event"].append(logEvent)
                    experimentLog["Time"].append(str(logEventTime))

                # Update the values of the optimisation objectives which are equal to the minimised inputs
                for key in self.main.methodHandler.inputsDict.keys():
                    optimisationSummary[f'{key}_min'].append(optimisationSummary[key][-1])

                # newSurfactantConcMin = optimisationSummary["Surfactant_concentration"][i]
                # newSeedFracMin = optimisationSummary["Seed_fraction"][i]
                # optimisationSummary["Surfactant_concentration_min"].append(newSurfactantConcMin)
                # optimisationSummary["Seed_fraction_min"].append(newSeedFracMin)

                ''' Clean with solvent '''
                emulsionValve.stopDistribution()
                emulsionValve.valveSwitch(position=2) # Send solvent into CSTR 1
                solventValve.valveSwitch(position=2) # Switch solvent inlet

                print('Starting cleaning stage 2 ...')
                startClean2Time = datetime.datetime.now()
                logEvent = "Cleaning with solvent after reaction " + sampleID
                logEventTime = startClean2Time
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))
                while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startClean2Time).seconds < 60*cleanTime:
############
                    self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeCleaningStep/(60*cleanTime))
                    self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                    time.sleep(1)

                if self.main.methodHandler.manualStop:
                    break

                ''' Rinse with surfactant solution '''
                solventValve.valveHome() # Switch to surfactant solution

                print('Starting cleaning stage 3 ...')
                startClean3Time = datetime.datetime.now()
                logEvent = f"Refilling with surfactant solution after reaction {sampleID}"
                logEventTime = startClean3Time
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))
                while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startClean3Time).seconds < 60*cleanTime:
############
                    self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeCleaningStep/(60*cleanTime))
                    self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                    time.sleep(1)

                solventPump.stop()
                
                if self.main.methodHandler.manualStop:
                    break

                if not self.main.methodHandler.stopThread and (reactionsCounter) % (7*sampleStrategy) == 0:
                    self.main.methodHandler.pauseExperiment = True
                    self.main.methodHandler.dialogueLabel.setText("Experiment paused, clean sample bottles and press OK to continue")
                    self.main.methodHandler.dialogueLabel.setHidden(False)
                    self.main.methodHandler.ackButton.setHidden(False)
                    sampleLoopCounter += 1 # iterate sampleLoopCounter so the next experiment takes the sample in the correct sampling port
                    while self.main.methodHandler.pauseExperiment:
                        time.sleep(1)

                print(f'optimisationSummary = {optimisationSummary}')
                pd.DataFrame.from_dict(optimisationSummary).to_excel(f'{self.main.methodHandler.savePath}-optimisationSummary.xlsx') # Save optimisation summary to save folder
                pd.DataFrame.from_dict(experimentLog).to_excel(f'{self.main.methodHandler.savePath}-experimentLog.xlsx') # Save experiment log to save folder

                self.main.methodHandler.optIterations +=1

            print("COMPLETED")
            pd.DataFrame.from_dict(experimentLog).to_excel(f'{self.main.methodHandler.savePath}-experimentLog.xlsx') # Save experiment log to save folder
            self.main.methodHandler.stopExperiment()


    def runKinetics(self):
        print("RUNNING KINETICS")
        expID = self.main.methodHandler.experimentIDLineEdit.text()
        expIndex = self.main.methodHandler.expIndex
        parameters = self.main.methodHandler.variablesDict

        nSamples = int(self.main.methodHandler.numberKineticsSamplesLineEdit.text())

        reactionsCounter = 0
        sampleLoopCounter = 0
        kineticsSummary = {}
        experimentLog = {}
        
        ### Extract parameters
        volumeCSTR = parameters.get("volumeCSTR")
        numberCSTRs = parameters.get("numberCSTRs")
        volumeReactor2 = parameters.get("volumeReactor2")
        deadVolume = parameters.get("deadVolume")[0]
        cleanRate = parameters.get("cleanRate")[0]
        cleanTime = parameters.get("cleanTime")[0]
        cleanVolume = parameters.get("cleanVolume")[0]
        nEmulsionFeeds = parameters.get("numsFeeds")
        nEmulsionFeedsActive = max(nEmulsionFeeds)

        ### Name all equipment
        seedPump = self.main.controller.pump1
        monomerAPump = self.main.controller.pump2
        monomerBPump = self.main.controller.pump3
        surfactantPump = self.main.controller.pump12
        initiatorPump = self.main.controller.pump11
        waterPump = self.main.controller.pump9
        solventPump = self.main.controller.pump6

        solventValve = self.main.controller.valve1
        emulsionValve = self.main.controller.valve2
        kineticsCollectValve = self.main.controller.valve3
        outletValve = self.main.controller.valve5

        self.DLS = DLS_handler.StoppedFlowDLS(self, main=self.main)

        ### Get flowrates for pre-programmed conditions (OFAAT, DoE, LHC or first optimisation experiment)
        v_seed = self.main.methodHandler.flowRates.get("v_seed")
        v_monomerA = self.main.methodHandler.flowRates.get("v_monomerA")
        v_monomerB = self.main.methodHandler.flowRates.get("v_monomerB")
        v_surfactant = self.main.methodHandler.flowRates.get("v_surfactant")
        v_initiator = self.main.methodHandler.flowRates.get("v_initiator")
        v_water = self.main.methodHandler.flowRates.get("v_water")

        ######################
        ###   PREPARATION  ###
        ######################

        ''' Start main script '''
        print(f'Starting experiment at {str(datetime.datetime.now())}')
        logEvent = f"Starting experiment {str(expID)}"
        logEventTime = str(datetime.datetime.now())
        experimentLog["Event"] = list([logEvent])
        experimentLog["Time"] = list([logEventTime])
######
        # outletValve.valveHome() # set outlet to waste
        emulsionValve.valveHome() # set emulsion valve to waste
        kineticsCollectValve.valveHome() # set collection valve to waste
        # solventValve.valveHome() # set solvent to surfactant position

        ''' Run solvent pump '''
        solventPump.setFlowrateText.setText(str(cleanRate))
        solventPump.setFlowrate()
        solventPump.start()
######        
        print('Priming emulsion feeds...')
        ''' Prime emulsion feeds with surfactant solution (10 s for each) '''
        for i in range(2, int(nEmulsionFeedsActive) + 2):
            emulsionValve.valveSwitch(position=i)
            time.sleep(5)
        for i in reversed(range(2, int(nEmulsionFeedsActive) + 2)):
            emulsionValve.valveSwitch(position=i)
            time.sleep(5)

        solventPump.stop()

        ''' Update waste volume available '''
        volumePriming = 2*20*nEmulsionFeedsActive*(cleanRate/60)
        self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - volumePriming
        self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))

        emulsionValve.distributionModeCheckbox.setChecked(True) # Activate distribution mode on emulsion feed valve

        while not self.main.methodHandler.manualStop:
            ''' First calculate the total flow rates, time required to steady-state, and waste volume space required '''
            total_flowrate_emulsion = v_monomerA[0] + v_surfactant[0] + v_initiator[0] + v_water[0]
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Add monomer B flowrate if two monomer feeds being used
                total_flowrate_emulsion += v_monomerB[0]
            total_flowrate_i = v_seed[0] + total_flowrate_emulsion

            time_SS = 3*((volumeCSTR[0]*numberCSTRs[0]) + volumeReactor2[0] + deadVolume)/total_flowrate_i # Time to reach steady state (min)

            wasteVolumeReaction = time_SS*total_flowrate_i # Waste volume generated from getting to steady state (mL)
            wasteVolumeCleaningStep = cleanVolume # Waste volume generated from a single cleaning step
            wasteVolumeRequired = wasteVolumeReaction + 3*wasteVolumeCleaningStep # Total volume of waste generated in the i'th experiment (mL)

            preparationTime = (60*3*volumeCSTR[0])/(10*(total_flowrate_emulsion))
            sampleTime = 60*parameters.get("volumeSample")[0]/total_flowrate_i

            if wasteVolumeRequired > self.main.methodHandler.wasteVolumeAvailable:
                self.main.methodHandler.pauseExperiment = True
                self.main.methodHandler.dialogueLabel.setText("WARNING - waste bottle too full to complete\nthe next experiment, please empty it to continue")
                self.main.methodHandler.dialogueLabel.setHidden(False)
                self.main.methodHandler.ackButton.setHidden(False)
                while self.main.methodHandler.pauseExperiment:
                    time.sleep(1)
                self.main.methodHandler.wasteVolumeAvailable = 970 # Assumes waste bottle has been emptied and resets to full capacity
            
            ''' Update waste bottle volume available '''
            self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))

            seedVolumeRequired = (10*v_seed[0]*preparationTime/60) + (v_seed[0]*time_SS) + (v_seed[0]*sampleTime*nSamples)
        
            if seedVolumeRequired > self.main.methodHandler.seedVolumeAvailable:
                self.main.methodHandler.pauseExperiment = True
                self.main.methodHandler.dialogueLabel.setText("WARNING - seed syringe too empty for the next reaction, please refill and press continue")
                self.main.methodHandler.dialogueLabel.setHidden(False)
                self.main.methodHandler.ackButton.setHidden(False)
                while self.main.methodHandler.pauseExperiment:
                    time.sleep(1)
                self.main.methodHandler.seedVolumeAvailable = 95 # Assumes waste bottle has been emptied and resets to full capacity

            ''' Prepare emulsion at fast flow rate before starting reaction and start filling reactor with seed '''
            print(f'Preparing emulsion')
###########           
            emulsionValve.valveHome()
            seedPump.setFlowrateText.setText(str(np.round(10*v_seed[0], 5)))
            monomerAPump.setFlowrateText.setText(str(np.round(10*v_monomerA[0], 4)))
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.setFlowrateText.setText(str(np.round(10*v_monomerB[0], 4)))
            surfactantPump.setFlowrateText.setText(str(np.round(10*v_surfactant[0], 4)))
            initiatorPump.setFlowrateText.setText(str(np.round(10*v_initiator[0], 4)))
            waterPump.setFlowrateText.setText(str(np.round(10*v_water[0], 4)))
#######
            seedPump.setFlowrate()
            monomerAPump.setFlowrate()
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.setFlowrate()
            initiatorPump.setFlowrate()
            surfactantPump.setFlowrate()
            waterPump.setFlowrate()
            
            seedPump.start()
            monomerAPump.start()
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.start()
            initiatorPump.start()
            surfactantPump.start()
            waterPump.start()

            emulsionValve.valveHome()
##########
            ''' Calculate and start timer to prepare emulsion (3 CSTR volumes at preparation flow rate) '''
            startPreparationTime = datetime.datetime.now()
            while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startPreparationTime).seconds < preparationTime:
#############
                self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (10*v_seed[0]/60)
                self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                time.sleep(1)

            logEvent = f"Starting reaction"
            logEventTime = datetime.datetime.now()
            experimentLog["Event"].append(logEvent)
            experimentLog["Time"].append(str(logEventTime))

            if self.main.methodHandler.manualStop:
                break

            ''' Set flow rates '''
            seedPump.setFlowrateText.setText(str(v_seed[0]))
            monomerAPump.setFlowrateText.setText(str(v_monomerA[0]))
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.setFlowrateText.setText(str(v_monomerB[0]))
            surfactantPump.setFlowrateText.setText(str(v_surfactant[0]))
            initiatorPump.setFlowrateText.setText(str(v_initiator[0]))
            waterPump.setFlowrateText.setText(str(v_water[0]))
 ######### 
            seedPump.setFlowrate()
            monomerAPump.setFlowrate()
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.setFlowrate()
            initiatorPump.setFlowrate()
            surfactantPump.setFlowrate()
            waterPump.setFlowrate()
#########
            emulsionValve.nFeedsLineEdit.setText(str(nEmulsionFeeds[0])) # Set number of feeds for emulsion distribution
            emulsionValve.startDistribution()
#########
            print(f"Starting synthesis, steady state expected at {str(datetime.datetime.now() + datetime.timedelta(seconds=60*time_SS))}")

            '''Wait for steady state'''
            start_exp_time = datetime.datetime.now()
            while not self.main.methodHandler.stopThread and (datetime.datetime.now() - start_exp_time).seconds < 60*time_SS:
##############
                self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeReaction/(60*time_SS))
                self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (v_seed[0]/60)
                self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                time.sleep(1)
            if self.main.methodHandler.manualStop:
                break
#########                
            kineticsSummary["Sample"] = []
            kineticsSummary["Particle_size"] = []
            nEmulsionFeedsTemp = nEmulsionFeeds[0]
            for s in range(0, nSamples):
                sampleID = str(s)
                kineticsSummary["Sample"].append(s)
                if s > nSamples - nEmulsionFeedsTemp:
                    emulsionValve.stopDistribution() # Pause the emulsion distribution briefly
                    nEmulsionFeedsTemp -= 1 # Update the new number of CSTRs to feed the emulsion into
                    print(f'New number of emulsion feeds = {nEmulsionFeedsTemp}')
                    emulsionValve.nFeedsLineEdit.setText(str(nEmulsionFeedsTemp)) # Update the new number of emulsion feeds in the valve widget
                    emulsionValve.startDistribution() # Restart the emulsion feeding into the new number of CSTRs
                    flowRateFactor = nEmulsionFeedsTemp/nEmulsionFeeds[0] # Reduce the emulsion flow rate to reflect the new number of emulsion feeds using the 'flowRateFactor'

                    monomerAPump.setFlowrateText.setText(str(flowRateFactor*v_monomerA[0]))
                    if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                        monomerBPump.setFlowrateText.setText(str(flowRateFactor*v_monomerB[0]))
                    surfactantPump.setFlowrateText.setText(str(flowRateFactor*v_surfactant[0]))
                    initiatorPump.setFlowrateText.setText(str(flowRateFactor*v_initiator[0]))
                    waterPump.setFlowrateText.setText(str(flowRateFactor*v_water[0]))
############                
                    monomerAPump.setFlowrate()
                    if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                        monomerBPump.setFlowrate()
                    initiatorPump.setFlowrate()
                    surfactantPump.setFlowrate()
                    waterPump.setFlowrate()

                    total_flowrate_i = v_seed[0] + total_flowrate_emulsion*flowRateFactor # Update total flow rate for sample volume collection and DLS dilution calculation
                    sampleTime = 60*parameters.get("volumeSample")[0]/total_flowrate_i
                else: 
                    flowRateFactor = 1

                kineticsCollectPosition = s + 1

                kineticsCollectValve.valveSwitch(position=kineticsCollectPosition)
                print(f'kineticsCollectValve switched to position {kineticsCollectPosition}')
                if kineticsCollectPosition == 1:
                    kineticsCollectDelay = 30
                else:
                    kineticsCollectDelay = (60*3*0.1)/total_flowrate_i

                print(f'kineticsCollectDelay = {kineticsCollectDelay}')    
                startKineticsDelayTime = datetime.datetime.now()
                while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startKineticsDelayTime).seconds < kineticsCollectDelay + 60:
############
                    self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (v_seed[0]/60)
                    self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                    time.sleep(1)
############
                print("Sampling for DLS")
############
                self.DLS.sampleAndScan(concentration=parameters.get("w_f")[0], flowRate=total_flowrate_i, sampleID=sampleID)
############
                logEvent = f"DLS sample {sampleID} extracted"
                logEventTime = datetime.datetime.now()
                experimentLog["Event"].append(logEvent)
                experimentLog["Time"].append(str(logEventTime))
############    
                # outletValve.valveSwitch(position=(s+2))
                print(f"Outletvalve switched to position {s+2}")

############                
                total_flowrate_i = v_seed[0] + total_flowrate_emulsion*flowRateFactor
                sampleTime = 60*parameters.get("volumeSample")[0]/total_flowrate_i

                startSampleTime = datetime.datetime.now()
                while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startSampleTime).seconds < sampleTime:
############
                    self.main.methodHandler.seedVolumeAvailable = self.main.methodHandler.seedVolumeAvailable - (v_seed[0]/60)
                    self.main.methodHandler.seedVolumeCounter.setText(str(round(self.main.methodHandler.seedVolumeAvailable, 2)))
                    time.sleep(1)
                
                if self.DLS.DLSDataCollected == 1: # Check if DLS data has been saved and add size value to kineticsSummary
                    newSize = self.DLS.zAve
                    kineticsSummary["Particle_size"].append(newSize)
                    logEvent = f"DLS data saved for {sampleID}"
                    logEventTime = datetime.datetime.now()
                    experimentLog["Event"].append(logEvent)
                    experimentLog["Time"].append(str(logEventTime))
                else: 
                    print("Warning: no DLS data collected for this experiment")
                    newSize = random.randint(0, 10)
                    kineticsSummary["Particle_size"].append(newSize)
                    logEvent = f"DLS data saved for {sampleID}"
                    logEventTime = datetime.datetime.now()
                    experimentLog["Event"].append(logEvent)
                    experimentLog["Time"].append(str(logEventTime))
                    '''Start DLS analysis - extract sample, dilute, and pause flow'''

                if self.main.methodHandler.manualStop:
                    break

            ''' Sample collected, start cleaning cycle '''
#############            
            # outletValve.valveHome()
            seedPump.stop()
            monomerAPump.stop()
            if self.main.methodHandler.monomerRatioCheckBox.isChecked(): # Only try send flow rate commands to monomer B if monomer ratio is being varied.
                monomerBPump.stop()
            initiatorPump.stop()
            surfactantPump.stop()
            waterPump.stop()
#############
            ''' Clean with surfactant solution including emulsion feeds '''
            solventPump.start()
############
            print('Starting cleaning stage 1 ...')
            startClean1Time = datetime.datetime.now()
            logEvent = f"Cleaning with surfactant solution after reaction {sampleID}"
            logEventTime = startClean1Time
            experimentLog["Event"].append(logEvent)
            experimentLog["Time"].append(str(logEventTime))
            while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startClean1Time).seconds < 60*cleanTime:
#############
                self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeCleaningStep/(60*cleanTime))
                self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                time.sleep(1)

            if self.main.methodHandler.manualStop:
                break

            # Update the values of the optimisation objectives which are equal to the minimised inputs

            ''' Clean with solvent '''
            emulsionValve.stopDistribution()
            emulsionValve.valveSwitch(position=2) # Send solvent into CSTR 1
            solventValve.valveSwitch(position=2) # Switch solvent inlet

            print('Starting cleaning stage 2 ...')
            startClean2Time = datetime.datetime.now()
            logEvent = "Cleaning with solvent after reaction"
            logEventTime = startClean2Time
            experimentLog["Event"].append(logEvent)
            experimentLog["Time"].append(str(logEventTime))
            while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startClean2Time).seconds < 60*cleanTime:
############
                self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeCleaningStep/(60*cleanTime))
                self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                time.sleep(1)

            if self.main.methodHandler.manualStop:
                break

            ''' Rinse with surfactant solution '''
            solventValve.valveHome() # Switch to surfactant solution

            print('Starting cleaning stage 3 ...')
            startClean3Time = datetime.datetime.now()
            logEvent = f"Refilling with surfactant solution after reaction"
            logEventTime = startClean3Time
            experimentLog["Event"].append(logEvent)
            experimentLog["Time"].append(str(logEventTime))
            while not self.main.methodHandler.stopThread and (datetime.datetime.now() - startClean3Time).seconds < 60*cleanTime:
#############
                self.main.methodHandler.wasteVolumeAvailable = self.main.methodHandler.wasteVolumeAvailable - (wasteVolumeCleaningStep/(60*cleanTime))
                self.main.methodHandler.wasteVolumeCounter.setText(str(round(self.main.methodHandler.wasteVolumeAvailable, 1)))
                time.sleep(1)

            solventPump.stop()
            
            if self.main.methodHandler.manualStop:
                break

            pd.DataFrame.from_dict(experimentLog).to_excel(f'{self.main.methodHandler.savePath}-experimentLog.xlsx') # Save experiment log to save folder
            pd.DataFrame.from_dict(kineticsSummary).to_excel(f'{self.main.methodHandler.savePath}-kineticsSummary.xlsx') # Save experiment log to save folder
