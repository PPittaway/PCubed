...
#         intensityData = np.array([])
#         sizeAxis = np.array([])
#         self.data_line = self.dataFeed.plot(sizeAxis, intensityData, pen='r')
...
#         self.DLSsavePath = self.main.methodHandler.directoryLineEdit.text()

#         self.scanning = 0
#         self.DLSDataCollected = 0
    
# ### Function for diverting and diluting a product sample to the DLS - takes arguments of product concentration (w/w) and product flow rate (mL/min) to return DLSPump flow rate ###
...
#         DLSdeadVolume = 1.5 # dead volume between DLS mixing point and end of flow cell in mL
...
#         DLSdilutionWaitTime = polymerReplacementVolume/flowRate
...
#         DLSsteadyStateTime = 3.5*60*DLSdeadVolume/self.DLSflowRate

#         # Direct flow to DLS and dilute with DLS pump
# ##############################################################################################################
...
#         DLSsamplingTimeStart = datetime.now()
#         while not self.main.methodHandler.stopThread and (datetime.now() - DLSsamplingTimeStart).seconds < DLSsteadyStateTime:
#             time.sleep(1)
...
# ##############################################################################################################
#         # Start DLS scanning loop in separate thread - current function exits so other processes (i.e. reactor cleaning) continue
#         scanThread = threading.Thread(target=self.startScan)
#         scanThread.start()
#         return

#     ### Start thread for running DLS measurement ###
#     def startScan(self):
#         # Hold material in the flow cell for at least 2 mins to allow a full scan to complete
#         DLSholdTimeStart = datetime.now()
#         print(f'Holding sample in DLS cell whilst new measurement is completed')
#         while not self.main.methodHandler.stopThread and (datetime.now() - DLSholdTimeStart).seconds < 150:
#             time.sleep(1)
...
#         while not self.DLSstopThread and self.DLSDataCollected == 0:
#             self.getData()
#             time.sleep(1)
#         self.scanning = 0
...
#             print("DLS data saved for " + str(self.sampleID))
...
class StoppedFlowDLS(QtWidgets.QWidget):
    def __init__(self, parent, main):
        ...
        self.DLSfindPath = r'C:\Users\Pcubed\Documents\Peter\Online-DLS\Data-hold'
        self.DLSsavePath = r'C:\Users\Pcubed\Documents\Alex W\RAFT_emulsion\240416\240416_DoE_DP_RT'
...
### Start thread for running DLS measurement ###
    ...
    #     # Hold material in the flow cell for at least 2 mins to allow a full scan to complete
    #     DLSholdTimeStart = datetime.now()
    #     while not self.main.methodHandler.stopThread and (datetime.now() - DLSholdTimeStart).seconds < 120:
    #         print(f'Holding sample in DLS cell whilst new measurement is completed')
    #         time.sleep(1)
    #     print('Starting DLS scan')
    #     self.DLSstopThread = False
    ...
    def getData(self):
        ...
        if (self.startScanTime - fileCreateTime).seconds < 120:
            ...
            sizeDistDF = pd.DataFrame([self.particleSize, self.intensityDist, self.volumeDist, self.numberDist]).T
            sizeDistDF.columns = ["Particle size (nm)", "Intensity average", "Volume average", "Number average"]
            # self.DLSsaveName = self.main.methodHandler.experimentIDLineEdit.text()
